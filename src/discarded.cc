...
TTable* runMooreModel1(Configuration& config, Corpus& corpus, const int MAXLEN, TTable* oldDict){
  // reusable arrays
  ...
  if (oldDict == 0){
    if (config.useMooreLLREstimate){
      ...
      oldDict = new TTable(2, dictParamRank);

      int* sourceOccur = new int[dictParamRank[0]];         // number of sentences which contain target word (not number of occurrences!)
	...
	for (int j=0; j<targetBagSize; j++){
      ...
      fprintf(stderr, "  Computing log-likelihood scores\n");
      TTable* logLikelihood = new TTable(2, dictParamRank);
	...
	//	      logLikelihood->increment(t, 2, stat);
	if (doDump){
  ...
  for (int iteration=1; iteration<config.model1Iterations; iteration++){
    TTable* newDict = new TTable(2, dictParamRank);
    ...
    fprintf(stderr, "(Moore) Model 1 iteration %d\n", iteration+1);
	...
	int e_i=(i==0)?0:b->getSourceWordAt(i);
	sourceOccur[e_i]++;
      }
...
// 	}
	...
	//	t[0] = 0;
	e_i = 0;
	newDict->increment(e_i, f_j, trans[0][j]/sum);
	for (int i=1; i<=sourceLen; i++){
	  //	  t[0] = b->getSourceWordAt(i);
	  e_i = b->getSourceWordAt(i);
	  newDict->increment(e_i, f_j, trans[i][j]/sum);
	}
      }
    ...
    if (config.useLLRReestimation){
      TTable* logDict = new TTable(2, dictParamRank);
	...
	for (int j=1; j<=targetLen; j++){
	  ...
	  //	  t[0] = 0;
	  e_i = 0;
	  // calculate unnormalized prob for t(f_j | e_0);
	  //	  if (logDict->getStat(t) == 0){
	  if (logDict->getStat(e_i, f_j) == 0){
	    //	    double a = newDict->getStat(t);
	    //	    double ab = newDict->getStat(t, 1);
	    double a = newDict->getStat(e_i, f_j);
	    double ab = newDict->getStat(e_i);
	    double ac = targetOccur[f_j];
	    double b = ab-a;
	    double c = ac-a;
	    ...
	    if (llScore > 0){
	      logDict->increment(e_i, f_j, llScore);
	      llSum[0] += llScore;
	      if (llSum[0] > maxLLSum){ maxLLSum = llSum[0]; maxLLIndex=0; }
	    }
	  }

	  // calculate unnormalized prob for t(f_j | e_i);
	  for (int i=1; i<=sourceLen; i++){
	    //	    t[0] = b->getSourceWordAt(i);
	    e_i = b->getSourceWordAt(i);
// 	    if (logDict->getStat(t) == 0){
...
// 	      double ab = newDict->getStat(t, 1);
		...
		logDict->increment(e_i, f_j, llScore);
		llSum[e_i] += llScore;
		if (llSum[e_i] > maxLLSum){ maxLLSum = llSum[e_i]; maxLLIndex = e_i; }
	      }
	    }
	  }
	}
	...
	fclose(dumpFile);
      ...
      delete newDict;
      delete[] llSum;
      newDict = new TTable(2, dictParamRank);
	...
	for (int j=1; j<=targetLen; j++){
	  f_j = b->getTargetWordAt(j);
	  e_i = 0;
	  if (newDict->getStat(e_i, f_j) == 0 &&
	      logDict->getStat(e_i, f_j) > 0){
	    newDict->increment(e_i, f_j, logDict->getStat(e_i, f_j)/maxLLSum);
	  }
	  for (int i=1; i<=sourceLen; i++){
	    e_i = b->getSourceWordAt(i);
	    if (newDict->getStat(e_i, f_j) == 0 &&
...
//       t[0]=0;
//       for (int j=1; j<dictParamRank[1]; j++){
// 	t[1]=j;
// 	newDict->increment(t, 2, targetOccur[j]/totalLinks);
//       }
      delete logDict;
    } else {
      newDict->normalize();
